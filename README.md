...
### Register the webapp (python-django-webapp-auth-my-tenant)
...
### Configure the webapp (python-django-webapp-auth-my-tenant) to use your app registration
...
In `my_tenant/msal_middleware.py`'s `MsalMiddleware` class:

1. A configuration object is parsed from [aad.config.json](./aad.config.json)
1. A DjangoAdapter is instantiated for interfacing with the Django app
1. The DjangoAdapter and an Azure AD configuration object are used to instantiate **IdentityWebPython**.
