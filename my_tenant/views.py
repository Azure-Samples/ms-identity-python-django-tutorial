...
from my_tenant.msal_middleware import ms_id_web, config
import logging
...
def aad_redirect(request):
    ...
    return ms_id_web.process_auth_redirect(next_action, # TODO: remove 'next_action' -> add redirect function to django adapter?
        redirect_uri=request.build_absolute_uri(reverse(endpoints.redirect))) 


def sign_out(request):
    logger.debug(f"{prefix}{endpoints.sign_out}: signing out username: {request.identity_context_data.username}")
    return ms_id_web.sign_out(request.build_absolute_uri(reverse(endpoints.post_sign_out)))    # send the user to Azure AD logout endpoint
...
def post_sign_out(request):
    ...
    ms_id_web.remove_user(request.identity_context_data.username)  # remove user auth from session on successful logout
    return redirect(reverse('index'))                   # take us back to the home page

@ms_id_web.login_required
